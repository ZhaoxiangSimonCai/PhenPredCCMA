...
class MOSA(nn.Module):
    def __init__(
        ...
        self.return_for_shap_indices = None
        self.layer_idx_map = {
    ...
    def forward(self, x_all):
        ...
        if self.return_for_shap == "latent":
            ...
            out = x_hat[self.layer_idx_map[self.return_for_shap]]
            if self.return_for_shap_indices is not None:
                out = out[:, self.return_for_shap_indices]
            return out
        else:
            return dict(
                x_hat=x_hat,
    ...
    def loss(self, x, x_nans, out_net, y, x_mask, view_loss_weights=None):
        ...
        # KL divergence loss
        kl_loss = (
            CLinesLosses.kl_divergence(
                mu,
                logvar,
                clip_min=self.hypers.get("kl_logvar_clip_min"),
                clip_max=self.hypers.get("kl_logvar_clip_max"),
            )
            * self.hypers["w_kl"]
        )
        ...
        if len(self.hypers["labels"]) == 0 or float(self.hypers.get("w_contrastive", 0.0)) <= 0:
            c_loss = 0
            ...
            n_conditionals = int(y.shape[1]) if y.dim() > 1 else 1
            if n_conditionals == 0:
                c_loss = 0
            else:
                c_loss = [loss_func(mu, y[:, i]) for i in range(n_conditionals)]
