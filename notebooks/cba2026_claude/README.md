...
Companion to `docs/CBA_Abstract2026_v1.docx`. Two parallel suites for the
selected +CNV MOSA run `20260505_131645`:

- **SHAP suite** (Figs 1–8): which features the model learns to use, mapped
  back to known biology. Outputs at `reports/cba2026_claude/shap_analysis/20260505_131645/`.
- **Prediction suite** (Figs P1–P11): how well TabPFN+MOSA predicts vs RF on
  ...
  itself faithful. Outputs at `reports/cba2026_claude/prediction_analysis/20260505_131645/`.
...
Composite figures live directly in `reports/cba2026_claude/shap_analysis/<TIMESTAMP>/`. Single-panel slide-friendly versions of every panel live alongside in `…/singles/` (see below).
...
Composites at `reports/cba2026_claude/prediction_analysis/<TIMESTAMP>/`. Single-panel slide-friendly exports under `…/singles/`.
...
| `figP11_cnv_ablation.{png,pdf}` | CNV-vs-no-CNV MOSA — bar of mean Pearson r at the headline condition + paired per-target scatter. Compares `20260313_162348` (no-CNV) against `20260505_131645` (with-CNV). |
...
d = Path('reports/cba2026_claude/shap_analysis/20260505_131645')
